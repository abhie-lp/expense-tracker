...
def add_expense_view(request):
    ...
    last_month = Expense.objects.filter(
        date__month=last_month_date.month, date__year=last_month_date.year
    ).aggregate(Sum("amount"))

    year_month: YearMonth = year_month_before(3)
    last_3_months = Expense.objects.filter(
        date__month__gte=year_month.month, date__year__gte=year_month.year,
        date__month__lte=last_month_date.month, date__year__lte=last_month_date.year
    ).aggregate(Sum("amount"))

    this_year = Expense.objects.filter(date__year=today.year).aggregate(Sum("amount"))
    last_year = Expense.objects.filter(date__year=today.year - 1).aggregate(Sum("amount"))
